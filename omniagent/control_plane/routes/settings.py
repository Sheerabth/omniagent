...
from omniagent.control_plane.models import ApiKeyCreate, ApiKeyRecord
...
async def create_api_key(body: ApiKeyCreate, _=Depends(require_any)):
    key = generate_key()
...
async def list_api_keys(_=Depends(require_any)):
    async with get_conn() as conn:
        rows = await conn.execute(
...
async def delete_api_key(key_id: uuid.UUID, _=Depends(require_any)):
    async with get_conn() as conn:
